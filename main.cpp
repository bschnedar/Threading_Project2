...
void fifo_schedule(void)
{
	...
	while (1) {
		pthread_mutex_lock(&mutex);
		pthread_cond_signal(&cond[readyQue[fifo]]);
		pthread_mutex_unlock(&mutex);

		//printf("Period: %d	Deadline: %d total_time: %d\n", tcb[readyQue[fifo]].period, tcb[readyQue[fifo]].deadline, iterr * tcb[readyQue[fifo]].task_time);

		if (tcb[readyQue[fifo]].period  <= iterr * tcb[readyQue[fifo]].task_time ) {
			pthread_mutex_lock(&mutex);
			fifo++;
			iterr = 0;
			pthread_mutex_unlock(&mutex);
...
void edf_schedule(void)
{
	...
	for (int i = 0; i < 4; i++) {
		if (tcb[cc].deadline > tcb[i].deadline ) {
			pthread_mutex_lock(&mutex);
			cc++;
			pthread_mutex_unlock(&mutex);
		}
